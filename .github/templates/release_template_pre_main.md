...
- [ ] Run `./release start --steps 1-pre-check --target-version vX.Y.Z-pre.N`
  ...
  - [ ] Check with @cilium/security team in case there are any CVEs found in the
        docker image.
  - [ ] Check for any security patches under `label:release-blocker/X.Y` in the
        [cilium/security] repository that should be merged before performing the
        release.
...
[cilium/security]: https://github.com/cilium/security/issues?q=is%3Aissue+state%3Aopen+label%3Arelease-blocker%2FX.Y
